...
import com.ravi.musicstore.model.Slider;
import com.ravi.musicstore.repository.SliderRepository;
...
public class AdminSliderController {
    ...
    public String saveHomeSliderPOST(@Valid @ModelAttribute("slider") Slider slider, BindingResult result, 
    	...
    	if(slider.getSliderId() == null){
    		...
    		// create directory
            File theDir = new File( rootDirectory + "WEB-INF/resources/images/slider/" + slider.getSliderId().toString());
            try {
        ...
        Path path = Paths.get(rootDirectory + "WEB-INF/resources/images/slider/" + slider.getSliderId().toString() + "/0.png");
        ...
        if (sliderImage != null && !sliderImage.isEmpty()) {
            try {
                ...
                e.printStackTrace();
                throw new RuntimeException("Slider image saving failed." + e.getMessage(), e);
            }
        }
    ...
    @RequestMapping("/d")
    public String deleteHomeSlider(@RequestParam(value="id", required=true) Long sliderId, Model model, HttpServletRequest request){
        String rootDirectory = request.getSession().getServletContext().getRealPath("/");
        File theDir = new File(rootDirectory + "WEB-INF/resources/images/slider/" + sliderId);
